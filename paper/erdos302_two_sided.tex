...
external Problem 301/327 dependencies. The upper certificate, semantic
bridge, finite omission argument, and asymptotic endpoint are also checked
end to end in Lean 4. The lower statement is unconditional
...
and this manuscript are unrefereed. Neither result closes Problem 302.
\end{abstract}
...
Readers checking the lower formalization should go to a single declaration,
namely
\[
...
prefix-omission certificate, and the asymptotic endpoint.  Readers checking
the upper formalization should go to the declaration
\[
 \texttt{Erdos302.Asymptotic.f302\_upper\_140803024\_163562355}
\]
in the file \texttt{Erdos302/Asymptotic/\allowbreak Integration.lean}.  It
states that for every $\varepsilon>0$,
\[
 f(N)\leq\left(\frac{140803024}{163562355}+\varepsilon\right)N
\]
for all sufficiently large $N$, where $f$ is the literal finite maximum
defined in \texttt{Erdos302/\allowbreak Problem.lean}; it is the
specialization to that maximum of a theorem proved for every function
satisfying the interface predicate \texttt{IsMaxNoTripleCard}.  The final
theorem has no certificate hypothesis; its transitive axiom report contains
only
...
All project modules in this upper-theorem closure have committed Lean source.
The authoritative high-memory run recompiles 48 critical modules, then
Lean4Checker replays declarations in the rebuilt integration and asymptotic
modules against the imported environment. The cached 80,127-module immutable
SHA-256-verified \texttt{.olean} overlay and \texttt{.olean} serialization
remain trusted. No cache-free full-project rebuild has completed yet, and the
replay does not establish that every cached binary was produced from the
corresponding source in this commit. Thus ``source-closed'' here means source
availability, not complete source-to-binary provenance.
