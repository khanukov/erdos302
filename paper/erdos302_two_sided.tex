...
\date{Version 0.1.1-dev, 17 August 2026\\[0.5ex]
...
in the standard formal sense: CI fresh-replays its complete exact-pinned
stored dependency closure through the Lean kernel and reports no nonstandard
axioms. The replay structurally trusts the serialized \texttt{.olean} input.
...
proof term and complete transitive stored dependency closure, including the
analytic input, are replayed through Lean's kernel into a fresh environment.
...
current blocking fresh-replay step. The present workflow additionally runs
\texttt{lake env leanchecker {-}{-}fresh Erdos302Lower} over the complete top
module import closure. The upstream results are imported as proof terms, not
as hypotheses.
