...
mathematics has been reviewed by anyone.  If the pinned Problem 301
development is wrong, the lower bound of Theorem~\ref{thm:main} falls with
it.  The odd-quarter padding of Lemma~\ref{lem:padding}, which is the part
original to this work, contributes $N/8-O(1)$ elements to a construction
whose $(1/2+\rho_L/24)N$ bulk is entirely upstream, and does not survive on
its own.  The upper bound of Theorem~\ref{thm:main} has no such dependency.
...
\]
in the file \texttt{lower-lean/Erdos302Lower/Maximum.lean}.  It states
\[
 \exists\,\delta>0\ \exists\,N_0\ \forall N\geq N_0:\quad
 \left(\frac58+\delta\right)N\leq f_{302}(N),
\]
and unfolds to only two project definitions, \texttt{NoUnitFractionTriple}
and \texttt{f302}, both reproduced in the repository README.  Everything else
in the lower project exists to prove that one statement.
...
cover number is $k$ by construction.  This replaces an infeasible computation
on $H$ by $21$ feasible ones on $H_0$.
