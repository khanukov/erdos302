...
\date{Version 0.2.0-preprint, 12 September 2026\\[0.5ex]
...
imported structured input.  The ingredient developed in this work is
...
\begin{sloppypar}
The Lean development checks the finite semantic
data, all 271 natural-number packing certificates, the 274-level ledger, the
prefix-omission certificate, and the asymptotic endpoint.  The upper interface
theorem is
\path{Erdos302.Asymptotic.erdos_302_upper_140803024_163562355}; its
concrete extremal-function corollary is
\path{Erdos302.Asymptotic.f302_upper_140803024_163562355}.  The interface
theorem has no certificate hypothesis, and the concrete corollary has no
abstract-function or maximality premise.  Their transitive axiom reports contain
only \texttt{propext}, \texttt{Classical.choice}, and \texttt{Quot.sound}.  The
interface predicates \texttt{NoUnitFractionTriple} and
\texttt{IsMaxNoTripleCard} match
the corresponding Erd\H{o}s-302 definitions in
\path{google-deepmind/formal-conjectures} at the pinned source revision (up
