import Erdos302.CertificateNat

set_option maxRecDepth 1000
set_option maxHeartbeats 0

namespace Erdos302.Generated

def packingCertificateNat263VertexGroup96 : List Erdos302.PackingTermNat := [
  { configurationId := 12492, snapshot := { maximum := 699, demand := 1, support := [681, 694, 699] },
    numerator := 1280856500, denominator := 4058972567, units := 0 },
  { configurationId := 12495, snapshot := { maximum := 704, demand := 1, support := [686, 694, 704] },
    numerator := 303360750, denominator := 1704523223, units := 0 },
  { configurationId := 12517, snapshot := { maximum := 702, demand := 1, support := [686, 696, 702] },
    numerator := 595996625, denominator := 1434742569, units := 0 },
  { configurationId := 12578, snapshot := { maximum := 704, demand := 1, support := [691, 702, 704] },
    numerator := 121037875, denominator := 1434742569, units := 0 },
  { configurationId := 12689, snapshot := { maximum := 86, demand := 15, support := [0, 1, 2, 3, 4, 5, 6, 8, 10, 11, 12, 13, 15, 16, 18, 21, 22, 23, 25, 27, 28, 31, 36, 37, 41, 45, 47, 50, 54, 57, 58, 63, 68, 71, 80, 83, 86] },
    numerator := 4902800, denominator := 12262757, units := 0 },
]

def packingCertificateNat263VertexGroup97 : List Erdos302.PackingTermNat := [
  { configurationId := 12692, snapshot := { maximum := 129, demand := 18, support := [0, 1, 2, 3, 4, 5, 6, 8, 10, 11, 12, 13, 15, 16, 18, 21, 22, 23, 25, 27, 28, 31, 36, 37, 41, 45, 47, 50, 54, 57, 58, 63, 68, 71, 80, 83, 86, 92, 102, 113, 120, 129] },
    numerator := 6128500, denominator := 12262757, units := 0 },
  { configurationId := 12710, snapshot := { maximum := 234, demand := 15, support := [17, 24, 29, 35, 40, 44, 48, 56, 61, 67, 70, 73, 82, 85, 89, 97, 101, 104, 108, 115, 119, 125, 135, 139, 148, 156, 161, 168, 175, 180, 183, 194, 204, 208, 225, 230, 234] },
    numerator := 12257000, denominator := 12262757, units := 0 },
  { configurationId := 13028, snapshot := { maximum := 289, demand := 18, support := [14, 20, 26, 30, 34, 38, 42, 49, 55, 59, 62, 64, 74, 75, 81, 88, 90, 93, 98, 105, 106, 114, 123, 127, 136, 145, 147, 155, 162, 166, 170, 181, 190, 193, 209, 214, 220, 232, 245, 263, 274, 289] },
    numerator := 6128500, denominator := 12262757, units := 0 },
  { configurationId := 13047, snapshot := { maximum := 446, demand := 16, support := [78, 95, 110, 122, 133, 142, 150, 165, 177, 187, 192, 197, 213, 216, 227, 239, 244, 249, 256, 266, 270, 283, 295, 301, 313, 324, 329, 340, 348, 354, 359, 371, 383, 387, 405, 411, 417, 428, 446] },
    numerator := 6128500, denominator := 12262757, units := 0 },
  { configurationId := 13193, snapshot := { maximum := 309, demand := 15, support := [34, 46, 55, 62, 69, 75, 81, 90, 100, 106, 112, 114, 127, 130, 138, 147, 152, 155, 162, 170, 173, 182, 193, 200, 209, 220, 224, 233, 241, 245, 252, 263, 274, 279, 297, 302, 309] },
    numerator := 6128500, denominator := 12262757, units := 0 },
]

def packingCertificateNat263VertexGroup98 : List Erdos302.PackingTermNat := [
  { configurationId := 13262, snapshot := { maximum := 702, demand := 21, support := [270, 304, 329, 348, 362, 376, 387, 405, 421, 433, 439, 446, 463, 467, 477, 489, 494, 500, 507, 515, 519, 529, 539, 545, 554, 563, 566, 574, 580, 583, 588, 597, 604, 606, 618, 621, 625, 631, 638, 649, 654, 659, 664, 672, 680, 690, 702] },
    numerator := 6128500, denominator := 12262757, units := 0 },
  { configurationId := 14165, snapshot := { maximum := 622, demand := 21, support := [92, 113, 129, 143, 154, 164, 172, 188, 202, 212, 217, 223, 240, 243, 254, 265, 271, 278, 286, 296, 300, 312, 323, 330, 343, 355, 358, 369, 377, 382, 388, 401, 413, 416, 434, 440, 447, 458, 473, 490, 501, 511, 525, 540, 559, 584, 622] },
    numerator := 6128500, denominator := 12262757, units := 0 },
  { configurationId := 14186, snapshot := { maximum := 697, demand := 21, support := [246, 280, 303, 322, 337, 351, 361, 381, 396, 408, 415, 420, 438, 443, 454, 465, 472, 476, 484, 493, 497, 508, 517, 524, 535, 544, 547, 555, 562, 565, 570, 579, 587, 590, 603, 605, 611, 617, 627, 636, 642, 648, 655, 662, 671, 683, 697] },
    numerator := 12257000, denominator := 12262757, units := 0 },
  { configurationId := 14333, snapshot := { maximum := 665, demand := 21, support := [154, 181, 202, 217, 232, 243, 254, 271, 289, 300, 306, 312, 330, 334, 346, 358, 363, 369, 377, 388, 393, 404, 416, 422, 434, 447, 451, 461, 468, 473, 478, 490, 501, 504, 520, 525, 530, 540, 552, 567, 575, 584, 595, 607, 622, 639, 665] },
    numerator := 6128500, denominator := 12262757, units := 0 },
  { configurationId := 14480, snapshot := { maximum := 701, demand := 21, support := [266, 301, 324, 345, 359, 371, 383, 402, 417, 428, 436, 441, 459, 463, 474, 485, 491, 495, 503, 512, 515, 526, 536, 541, 551, 560, 563, 571, 577, 581, 585, 594, 601, 604, 615, 619, 623, 629, 637, 646, 652, 658, 663, 670, 679, 689, 701] },
    numerator := 6128500, denominator := 12262757, units := 0 },
]

def packingCertificateNat263VertexGroup99 : List Erdos302.PackingTermNat := [
  { configurationId := 14668, snapshot := { maximum := 700, demand := 20, support := [321, 356, 380, 398, 414, 426, 437, 455, 471, 483, 487, 492, 509, 513, 523, 534, 537, 542, 549, 556, 561, 569, 578, 582, 592, 598, 602, 609, 613, 616, 620, 628, 632, 635, 644, 647, 650, 656, 661, 669, 673, 677, 682, 688, 693, 700] },
    numerator := 12257000, denominator := 12262757, units := 0 },
]

def packingCertificateNat263VertexChunk24 : List Erdos302.PackingTermNat :=
  packingCertificateNat263VertexGroup96 ++ packingCertificateNat263VertexGroup97 ++ packingCertificateNat263VertexGroup98 ++ packingCertificateNat263VertexGroup99

end Erdos302.Generated
