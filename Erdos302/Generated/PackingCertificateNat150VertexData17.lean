import Erdos302.CertificateNat

set_option maxRecDepth 1000
set_option maxHeartbeats 0

namespace Erdos302.Generated

def packingCertificateNat150VertexGroup68 : List Erdos302.PackingTermNat := [
  { configurationId := 13111, snapshot := { maximum := 398, demand := 17, support := [49, 62, 74, 84, 90, 98, 105, 118, 127, 136, 141, 145, 160, 162, 170, 181, 186, 190, 196, 207, 209, 220, 232, 238, 248, 260, 263, 274, 282, 289, 294, 306, 317, 321, 339, 346, 352, 363, 380, 398] },
    numerator := 6884234000028, denominator := 33064676778857, units := 0 },
  { configurationId := 13196, snapshot := { maximum := 380, demand := 18, support := [34, 46, 55, 62, 69, 75, 81, 90, 100, 106, 112, 114, 127, 130, 138, 147, 152, 155, 162, 170, 173, 182, 193, 200, 209, 220, 224, 233, 241, 245, 252, 263, 274, 279, 297, 302, 309, 321, 336, 356, 366, 380] },
    numerator := 15027641992440, denominator := 33064676778857, units := 0 },
  { configurationId := 14078, snapshot := { maximum := 382, demand := 18, support := [36, 47, 57, 63, 71, 77, 83, 92, 102, 109, 113, 117, 129, 132, 140, 149, 154, 159, 164, 172, 176, 185, 195, 202, 212, 223, 226, 236, 243, 247, 254, 265, 278, 281, 300, 305, 312, 323, 338, 358, 369, 382] },
    numerator := 2185454545736, denominator := 33064676778857, units := 0 },
  { configurationId := 14206, snapshot := { maximum := 404, demand := 20, support := [18, 26, 31, 37, 42, 47, 50, 58, 64, 71, 74, 76, 86, 88, 93, 102, 105, 107, 113, 120, 123, 131, 140, 145, 154, 163, 166, 174, 181, 185, 190, 202, 210, 214, 232, 236, 242, 254, 268, 289, 298, 312, 326, 346, 369, 404] },
    numerator := 6810527350973808, denominator := 30981602141789009, units := 0 },
  { configurationId := 14372, snapshot := { maximum := 404, demand := 18, support := [42, 55, 64, 74, 81, 88, 93, 105, 114, 123, 127, 131, 145, 147, 155, 166, 170, 174, 181, 190, 193, 203, 214, 220, 232, 242, 245, 255, 263, 268, 274, 289, 298, 302, 321, 326, 332, 346, 360, 380, 390, 404] },
    numerator := 239690889779418, denominator := 1421781101490851, units := 0 },
]

def packingCertificateNat150VertexChunk17 : List Erdos302.PackingTermNat :=
  packingCertificateNat150VertexGroup68

end Erdos302.Generated
