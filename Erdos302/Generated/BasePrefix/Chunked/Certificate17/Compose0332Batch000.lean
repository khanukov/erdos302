import Erdos302.Generated.BasePrefix.Chunked.Certificate17.Validity0332
import Erdos302.Generated.BasePrefix.Chunked.Certificate17.Compose0192Root
import Erdos302.Generated.BasePrefix.Chunked.Certificate17.Compose0219Root
import Erdos302.Generated.BasePrefix.Chunked.Certificate17.Compose0292Root
import Erdos302.Generated.BasePrefix.Chunked.Certificate17.Compose0314Root
import Erdos302.Generated.BasePrefix.Chunked.Certificate17.Compose0319Root
import Erdos302.Generated.BasePrefix.Chunked.Certificate17.Compose0331Root

set_option maxHeartbeats 20000000

namespace Erdos302.Generated.BasePrefix.Chunked.Certificate17.Compose0332Batch000
open Erdos302.BasePrefixCoverChunk
open Erdos302.Generated.BasePrefix
open Erdos302.Generated.BasePrefix.Chunked.Certificate17

def sources : List (Claim 146) := [Validity0192.steps[6].claim, Validity0219.steps[30].claim, Validity0292.steps[3].claim, Validity0314.steps[16].claim, Validity0319.steps[4].claim, Validity0331.steps[7].claim, Validity0331.steps[19].claim, Validity0331.steps[22].claim, Validity0331.steps[29].claim, Validity0331.steps[30].claim, Validity0331.steps[31].claim]
theorem sources_match : SliceEq Validity0332.imports sources 0 := by decide
theorem sources_hold : ClaimsHold sources support := by
  intro claim h
  simp only [sources, List.mem_cons, List.not_mem_nil, or_false] at h
  rcases h with rfl | h
  exact Compose0192Root.all_holds ⟨6, by decide⟩
  rcases h with rfl | h
  exact Compose0219Root.all_holds ⟨30, by decide⟩
  rcases h with rfl | h
  exact Compose0292Root.all_holds ⟨3, by decide⟩
  rcases h with rfl | h
  exact Compose0314Root.all_holds ⟨16, by decide⟩
  rcases h with rfl | h
  exact Compose0319Root.all_holds ⟨4, by decide⟩
  rcases h with rfl | h
  exact Compose0331Root.all_holds ⟨7, by decide⟩
  rcases h with rfl | h
  exact Compose0331Root.all_holds ⟨19, by decide⟩
  rcases h with rfl | h
  exact Compose0331Root.all_holds ⟨22, by decide⟩
  rcases h with rfl | h
  exact Compose0331Root.all_holds ⟨29, by decide⟩
  rcases h with rfl | h
  exact Compose0331Root.all_holds ⟨30, by decide⟩
  rcases h with rfl
  exact Compose0331Root.all_holds ⟨31, by decide⟩
def slots : List Nat := List.range' 0 sources.length
theorem holds : ImportsHold Validity0332.imports support slots := by
  exact ImportsHold.ofSliceEq sources_match sources_hold

end Erdos302.Generated.BasePrefix.Chunked.Certificate17.Compose0332Batch000
