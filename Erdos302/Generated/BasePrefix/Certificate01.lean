...
namespace Erdos302.Generated.BasePrefix.Certificate01

open Erdos302.BasePrefixCover
open Erdos302.Generated.BasePrefix

def certificate : Array (Node 47 146) := #[
  { mask := 8, budget := 0, rule := .packing [3].toFinset },
  { mask := 1, budget := 0, rule := .packing [0].toFinset },
  { mask := 11, budget := 1, rule := .branch 1 [(1, 0), (2, 1), (8, 1)] }
]

def root : Fin certificate.size := ⟨2, by decide⟩
...
theorem root_semantics :
    activeEdges certificate[root].mask = prefixEdges 12 ∧
    certificate[root].budget + 1 = 2 := by decide

theorem demand_valid : PrefixDemand 12 2 := by
  intro cover hcover
...
end Erdos302.Generated.BasePrefix.Certificate01
